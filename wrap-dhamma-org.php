...
function wrap_dhamma_fetch_remote_content( $url, $cache_time = null ) {
	if ( null === $cache_time ) {
		$cache_time = get_option( 'wrap_dhamma_cache_duration', WRAP_DHAMMA_CACHE_EXPIRATION );
	}
...
function wrap_dhamma_fix_urls( $raw, $lang ) {
	...
	$raw = preg_replace( '#<a href=["\']/?code/?["\']>#', '<a href="' . esc_url( $home_url . '/courses/code/' ) . '">', $raw );
	$raw = str_replace( "<a href='/bycountry/'>", '<a target="_blank" rel="noopener" href="https://courses.dhamma.org/en-US/schedules/schdhara">', $raw );
	$raw = str_replace( "<a href='/docs/core/code-en.pdf'>here</a>", '<a href="https://www.dhamma.org/en/docs/core/code-en.pdf">here</a>', $raw );
	$raw = str_replace(
